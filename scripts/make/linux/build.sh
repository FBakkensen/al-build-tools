#!/usr/bin/env bash
set -euo pipefail
# Linux Build Script - equivalent to Windows build.ps1
# Usage: build.sh [AppDir]

AppDir="${1:-app}"

# Source shared libraries
# shellcheck source=./lib/common.sh
# shellcheck disable=SC1091
source "$(dirname "$0")/lib/common.sh"
# shellcheck source=./lib/json-parser.sh
# shellcheck disable=SC1091
source "$(dirname "$0")/lib/json-parser.sh"

if ! type get_enabled_analyzer_paths &>/dev/null; then
    write_error_and_exit "ERROR: get_enabled_analyzer_paths is not available after import!"
fi

alc_path=$(get_al_compiler_path "$AppDir")
[[ -n "$alc_path" ]] || write_error_and_exit "AL Compiler not found. Please ensure AL extension is installed in VS Code."

mapfile -t analyzer_paths < <(get_enabled_analyzer_paths "$AppDir")

output_full_path=$(get_output_path "$AppDir")
[[ -n "$output_full_path" ]] || write_error_and_exit "[ERROR] Output path could not be determined. Check app.json and get_output_path function."

package_cache_path=$(get_package_cache_path "$AppDir")
[[ -n "$package_cache_path" ]] || write_error_and_exit "[ERROR] Package cache path could not be determined."

if [[ -f "$output_full_path" ]]; then
    rm -f "$output_full_path" || write_error_and_exit "[ERROR] Failed to remove ${output_full_path}"
fi
if [[ -d "$output_full_path" ]]; then
    rm -rf "$output_full_path" || write_error_and_exit "[ERROR] Failed to remove conflicting directory ${output_full_path}"
fi

app_json_path=$(get_app_json_path "$AppDir")
app_name=$(jq -r '.name // "Unknown App"' "$app_json_path" 2>/dev/null)
app_version=$(jq -r '.version // "1.0.0.0"' "$app_json_path" 2>/dev/null)
output_file=$(basename "$output_full_path")
echo -e "\033[0;32mBuilding ${app_name} v${app_version}...\033[0m"

if [[ ${#analyzer_paths[@]} -gt 0 ]]; then
    echo "Using analyzers from settings.json:"
    for analyzer_path in "${analyzer_paths[@]}"; do
        echo "  - $analyzer_path"
    done
    echo ""
else
    echo "No analyzers found or enabled in settings.json"; echo ""
fi

[[ -x "$alc_path" ]] || chmod +x "$alc_path"

cmd_args=("/project:$AppDir" "/out:$output_full_path" "/packagecachepath:$package_cache_path" "/parallel+")

if [[ -n "${RULESET_PATH:-}" ]]; then
    if [[ -f "$RULESET_PATH" && -s "$RULESET_PATH" ]]; then
        echo "Using ruleset: $RULESET_PATH"
        cmd_args+=("/ruleset:$RULESET_PATH")
    else
        echo "Ruleset not found or empty, skipping: ${RULESET_PATH}"
    fi
fi
for analyzer_path in "${analyzer_paths[@]}"; do
    cmd_args+=("/analyzer:$analyzer_path")
done

wae="${WARN_AS_ERROR:-0}"
shopt -s nocasematch
if [[ "$wae" == "1" || "$wae" == "true" || "$wae" == "yes" || "$wae" == "on" ]]; then
    cmd_args+=("/warnaserror+")
fi
shopt -u nocasematch

"$alc_path" "${cmd_args[@]}"
exit_code=$?
echo ""
if [[ $exit_code -ne 0 ]]; then
    echo -e "\033[0;31mBuild failed with errors above.\033[0m" >&2
else
    echo -e "\033[0;32mBuild completed successfully: $output_file\033[0m"
fi
exit $exit_code

