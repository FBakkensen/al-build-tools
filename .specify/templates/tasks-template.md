...
**Constitution Alignment**: Always include tasks that validate overlay contract safety, Windows/Linux parity, guarded execution (ALBT_VIA_MAKE + exit codes), deterministic provisioning, and copy-only release packaging when those concerns are in scope.
...
**Purpose**: Project initialization and basic structure that keeps overlay scripts self-contained and ready for parity validation
...
- [ ] T010 Validate Windows/Linux execution parity guardrails (if overlay scripts change)
- [ ] T011 Confirm deterministic provisioning steps and cache hygiene
