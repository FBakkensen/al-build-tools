...
function Install-AlBuildTools {
    ...
    Write-Host "[al-build-tools] Installing from $Url@$Ref into $Dest (source: $Source)" -ForegroundColor Cyan

    # Resolve destination to full path
    try {
        $destPath = Resolve-Path -Path $Dest -ErrorAction Stop
        $destFull = $destPath.Path
    } catch {
        New-Item -ItemType Directory -Force -Path $Dest | Out-Null
        $destFull = (Resolve-Path -Path $Dest).Path
    }

    # Non-fatal check for git repo
    $gitOk = $false
    try {
    ...
    if (-not $gitOk -and -not (Test-Path (Join-Path $destFull '.git'))) {
        Write-Warning "Destination '$destFull' does not look like a git repo. Proceeding anyway."
    }
    ...
    try {
        ...
        # Copy contents of Source into destination; overwrite existing files
        Copy-Item -Path (Join-Path $src '*') -Destination $destFull -Recurse -Force
        Write-Host "[al-build-tools] Copied '$Source' from $Url@$Ref into $destFull" -ForegroundColor Green
    } finally {
